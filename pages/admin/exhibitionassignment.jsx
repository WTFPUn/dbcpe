...
export default function exhibitionassignment() {
  ...
  useEffect(() => {
    ...
    if (token) {
      fetch('/api/admin/getexhibitionforupdate',
        {
          method: "GET",
          headers: {
      ...
      fetch('/api/exhibition/getexhibitiontype',
        ...
        .then((res) => res.json())
        .then((data) => {
          setexhibitionType(data.allexhibitiontype);
        }
        );
  ...
  const handleFilter = (e, filterkey) => {
    ...
    const token = localStorage.getItem("token");
    if (token) {
    fetch(`/api/admin/getexhibitionforupdate?${"clean_status=" + clean_status + "&roomtype_id=" + roomtype}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const handleSubmit = () => {
    const token = localStorage.getItem("token");
    if (token) {
      fetch('/api/admin/exhibitionupdateworkforhousekeeper', {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "auth-token": token,
  ...
  return(
    <Template hscreen>
      <div className="bg-[#F5F5F5] w-[60%] h-full mt-16 rounded-t-md px-[6rem] pt-8">
        <div className="w-full text-4xl font-semibold text-center h-[12%] pt-4 ">exhibition Assignment</div>
        <div className="mt-4 w-full h-[80%]  flex flex-col gap-8">
            ...
            <div className="w-[75%] flex flex-col gap-4">
              <div className="w-full h-1/2 rounded-md bg-[#FAFCFE] drop-shadow flex pl-4 place-items-center">
                ...
                <select className="w-full h-full rounded-md bg-[#FAFCFE] ml-4 appearance-none outline-none px-4 text-[#C4C4C4]" onChange={(e) => handleFilter(e, "roomtype")}>
                  ...
                  {exhibitionType.map((roomtype) => (
                    <option className="px-4 py-2" value={roomtype.exhibition_type_id}>{roomtype.type_name}</option>
                  ))}
                </select>
              </div>
          ...
          >
            <div className="flex font-semibold text-black h-[10%]">
              ...
              <div className="w-[15%] text-center">Room Name</div>
                ...
                return(
                  ...
                  <div className="w-[10%] flex items-center place-content-center">
                    <input
                      ...
                      onClick={() => setexhibition([...exhibition.map((roommap) => {
                        if (room.exhibition_id === roommap.exhibition_id) {
                  ...
                  <div className="w-[15%]  text-center">{room.exhibition_id}</div>
                  <div className="w-[15%]  text-center">{room.name}</div>
                  <div className="w-[15%]  text-center">{room.room_type.type_name}</div>
